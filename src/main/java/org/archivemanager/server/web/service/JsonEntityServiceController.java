...
public class JsonEntityServiceController extends WebserviceSupport {
	...
	public RestResponse<Object> associationAdd(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if(target == null) {
				...
				Association assoc = getAssociation(request, assocQname);
	...
	protected RestResponse<Object> getEntity(long id, String uid, String view, boolean sources, boolean targets) {
		RestResponse<Object> data = new RestResponse<Object>();
		try {
		...
		try {
			...
			if(parent == null || parent.equals("null")) {
				...
				if(mode != null && mode.equals("id")) {
					...
					if(parentEntity != null) nodes.add(parentEntity.getId());
	...
	protected Map<String,Object> getNodeData(Long node, boolean sources, boolean targets) {
		...
		try {
			...
			if(entity.hasProperty(SystemModel.NAME)) nodeData.put("name", entity.getProperty(SystemModel.NAME));
			else if(entity.hasProperty(SystemModel.DESCRIPTION)) nodeData.put("name", entity.getProperty(SystemModel.DESCRIPTION));
			else nodeData.put("name", entity.getQName().toString());
			if(entity.getSourceAssociations().size() > 0) nodeData.put("children", "true");
