...
public class Neo4jEntityService implements EntityService {
	...
	public List<Entity> getEntities(QName qname) {
		List<Long> ids = new ArrayList<Long>();
		List<Entity> entities = new ArrayList<Entity>();
		Session session = driver.session();
		StatementResult result = session.run("MATCH (n1) WHERE exists(n1.openapps_org_system_1_0_name) and n1.qname='"+qname.toString()+"' return n1;");
		try {
