 ...
 function loadAndOpen(qnameIn) {
     ...
     var panel = $('#collection-associations').accordion('getSelected');
     ...
     $('#addAssociationsWindow').window({ title: 'Add ' + panelTitle });
 ...
 function removeAssociation(assocDiv) {
         ...
         var checkedItems = $('#' + assocDiv).datalist('getChecked');
         var rootNode = $('#collectionTree').tree('getRoot');
         var selectItem = $('#collectionTree').tree('getSelected');
         var parentId = selectItem.id;
         checkedItems.forEach(
             function(item) {
                 var returnId;
                 var ajaxReq = $.ajax({
                     type: 'GET',
                     url: '/service/entity/get.json/' + item.id
                 });
                 ...
                 ajaxReq.done(function(result) {
                     ...
                     data.forEach(
                         function(parent) {
                             if (parent.parent.source == parentId && parent.parent.target == item.id) {
                                 returnId = parent.parent.id;
                                 var ajaxReq = $.ajax({
                                     type: 'POST',
                                 ...
                                 ajaxReq.done(function(result) {
                                     $('#collectionTree').tree('select', selectItem.target);
...
function removeEntity() {
    ...
    var rootNode = $('#collectionTree').tree('getRoot');
    if (node) {
        $.messager.confirm('Confirm', 'Are you sure you want to remove ' + node.text + '?', function(r) {
            if (r) {
                $.post('/service/entity/remove.json', { id: node.id }, function(result) {
                    if (result.response.status != 0) {
                        $.messager.show({ // show error message
                            ...
                            msg: result.errorMsg
                                    ...
                                    $('#collectionTree').tree('remove',node.target);
                                    rootNode = $('#collectionTree').tree('getRoot');
                                    $('#collectionTree').tree('reload',rootNode.target);
                    }
                }, 'json');
            }
        });
